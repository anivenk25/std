...
class StatisticalMethods:
    ...
    @staticmethod
    def predefined_analysis_methods(df: pd.DataFrame) -> List[str]:
        """List of predefined analysis methods to be called by LLM."""
        methods = [
            "StatisticalMethods.basic_stats(df)",
            "StatisticalMethods.normality_test(df['column_name'])",  # Replace 'column_name' with actual column
            "StatisticalMethods.outlier_detection(df['column_name'])",  # Replace 'column_name' with actual column
            "StatisticalMethods.dimension_reduction(df)"
        ]
        return methods
...
class LLMAnalyzer:
    ...
    def analyze_dataset(self, file_path: str):
        ...
        try:
            ...
            # Perform predefined analysis methods
            predefined_methods = StatisticalMethods.predefined_analysis_methods(df)
            print("\nCalling predefined analysis methods...")
            for method in predefined_methods:
                print(f"Executing: {method}")
                # Execute the method dynamically
                try:
                    exec(method)  # This will execute the method as a string
                except Exception as e:
